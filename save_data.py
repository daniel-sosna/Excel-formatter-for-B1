...
class LoadWorkbook():
    def __init__(self, filename, read_only=False):
        """ Load the workbook, retry if the file is open """
        while True:
            try:
...
class WriteSalesToExcel():
    def __init__(self, filename, sales, eu, not_eu):
        """ Initialize with sales data and write to Excel """
        self.all = sales
        self.eu = eu
        self.not_eu = not_eu
        self.write(filename)
    ...
    def write(self, filename):
        """ Write sales data to Excel workbook """
        workbook = Workbook()
...
class SaveData():
    def __init__(self, all_sales, EU_sales, not_EU_sales):
        """ Save modified sales data and fill out the template """
        ...
        if os.path.exists(TEMPLATE_PATH):
            ...
            print("[?] Enter the TEMPLATE FILE (or drag it into this window):")
            template_filename = input("» ").strip('"')
